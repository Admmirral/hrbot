...
# دیکشنری سریع
emote_dict = {e["text"].lower().replace(" ", ""): [e["value"], e["time"]] for e in timed_emotes}

class EmoteController:
    def __init__(self):
        self.current_emote_tasks = {}
        self.active_emotes = {}
        self.last_welcome_message_time = {}

    async def stop_current_emote(self, user_id, highrise=None):
        if user_id in self.current_emote_tasks:
            task = self.current_emote_tasks.pop(user_id)
            task.cancel()
            try:
                await task
                ...
                pass
            if highrise:
                await highrise.send_whisper(user_id, "ایموت شما متوقف شد ✅")
            self.active_emotes.pop(user_id, None)

    async def start_emote(self, highrise, user_id, emote_code, emote_duration, emote_name):
        await self.stop_current_emote(user_id, highrise)

        async def emote_loop():
            while user_id in self.current_emote_tasks:
                try:
                    await highrise.send_emote(emote_code, user_id)
                    await asyncio.sleep(emote_duration)
                except asyncio.CancelledError:
                    break
                except Exception as e:
                    print(f"⚠️ Error in emote loop: {e}")
                    break

        task = asyncio.create_task(emote_loop())
        self.current_emote_tasks[user_id] = task
        self.active_emotes[user_id] = emote_name

    async def send_welcome_message(self, highrise, user_id, user_username, emote_name):
        current_time = time.time()
        last_time = self.last_welcome_message_time.get(user_id, 0)
        if current_time - last_time >= COOLDOWN_TIME:
            self.last_welcome_message_time[user_id] = current_time
            welcome_message = f"@{user_username} ایموت '{emote_name}' اجرا شد ✅ (برای توقف: 'stop')"
            await highrise.send_whisper(user_id, welcome_message)

def wrap_on_chat(cls, new_on_chat):
    old_on_chat = getattr(cls, "on_chat", None)

    async def wrapped_on_chat(self, user, message):
        await new_on_chat(self, user, message)
        if old_on_chat:
            await old_on_chat(self, user, message)

    setattr(cls, "on_chat", wrapped_on_chat)
    return cls

def setup_bot(bot_class):
    async def new_on_chat(self, user: User, message: str) -> None:
        message_content = message.strip().lower().replace(" ", "")

        # فرمان‌های اسلش
    
        # توقف ایموت
        if message_content == "stop":
            await self.emote_controller.stop_current_emote(user.id, self.highrise)
            return

        # بررسی شماره یا اسم ایموت
        emote_name = None
        emote_names_list = list(emote_dict.keys())

        # آیا پیام عدد است؟
        try:
            emote_number = int(message_content)
            if 1 <= emote_number <= len(emote_names_list):
                emote_name = emote_names_list[emote_number - 1]
            else:
                await self.highrise.send_whisper(
                    user.id,
                    f"لطفاً عددی بین 1 تا {len(emote_names_list)} وارد کنید ❌"
                )
                return
        except ValueError:
            # بررسی بر اساس اسم ایموت
            if message_content in emote_dict:
                emote_name = message_content
            else:
                # اگر پیام اصلاً ایموت نیست، هیچ کاری نکن
                return

        # اجرای ایموت
        emote_info = emote_dict.get(emote_name)
        if not emote_info:
            await self.highrise.send_whisper(user.id, "ایموت پیدا نشد ❌")
            return

        emote_code = emote_info[0]
        emote_duration = emote_info[1] if len(emote_info) > 1 else DEFAULT_EMOTE_DURATION

        await self.emote_controller.start_emote(
            self.highrise,
            user.id,
            emote_code,
            emote_duration,
        ...
        await self.emote_controller.send_welcome_message(
            self.highrise, user.id, user.username, emote_name
        )

    class DecoratedBot(bot_class):
        def __init__(self, *args, **kwargs):
            super().__init__(*args, **kwargs)
            self.emote_controller = EmoteController()

    return wrap_on_chat(DecoratedBot, new_on_chat)
